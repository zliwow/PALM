...
def main(config):
    ...
    model = EncoderDecoderModel.from_pretrained(config.resume).to(device)

    log_example = []

    def seq_generate(input_seq, max_length, input_split_fn, input_tokenizer, target_tokenizer, beams, gene_num=16, gen_max_len=16, gen_min_len=14):
        ...
        outputs = model.generate(input_ids, 
                               ...
                               num_beams=16,  # Reduced from 1000 to 16 for MPS compatibility
                               do_sample=False,
                               diversity_penalty=0.05,
                               num_beam_groups=4,  # Reduced from 10 to 4
                               num_return_sequences=16,  # Reduced to match num_beams
    ...
    if model_input == 'antigen':
        ...
        selected_single_chain_df = pd.DataFrame({
            'antigen': used_antigen_list,
    ...
    # Generate full antibody sequences
